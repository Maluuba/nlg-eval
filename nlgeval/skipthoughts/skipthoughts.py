...
class Encoder(object):
    ...
    def encode(self, X, use_norm=True, verbose=True, batch_size=128):
        ...
        """
        return encode(self._model, X, use_norm, verbose, batch_size)
    ...
    for k in ds.keys():
        ...
        for minibatch in range(numbatches):
            ...
            for ind, c in enumerate(caps):
                ...
                if k == 0:
                    uembedding[-1, ind] = numpy.zeros(len(model['utable']['UNK']))
                    bembedding[-1, ind] = numpy.zeros(len(model['utable']['UNK']))

            if k == 0:
                uff = model['f_w2v'](uembedding, numpy.zeros((len(caption) + 1, len(caps)), dtype='float32'))
                bff = model['f_w2v2'](bembedding, numpy.zeros((len(caption) + 1, len(caps)), dtype='float32'))
            else:
